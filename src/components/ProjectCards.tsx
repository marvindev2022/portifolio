...
export default function ProjectCard({
  ...
  const { isDarkMode, assets } = useContextPage();
  return (
    <article
      className={`flex-col lg:flex-row lg:w-full lg:h-full flex ${
        isDarkMode
          ? "bg-dark border-white text-white"
          : "bg-white border-dark text-dark"
      }  border-[.0625rem]   lg:mt-[-.225rem] lg:ml-[-.225rem] rounded-lg ${
    ...
    >
      <img
        src={image}
        alt="imagem"
        className={`lg:w-3/6 lg:h-5/6 lg:object-cover rounded-lg m-5 ${
          isDarkMode ? "border-light" : "border-dark"
        } hover:scale-[1.03] transition-all duration-500 ease-in-out border-2`}
      />
      <div className="flex flex-col lg:w-1/2 lg:min-h-full  lg:object-fill  justify-around items-start  p-4 gap-5">
        <span
          className={`text-3xl ${
            isDarkMode ? "text-[aqua]" : "text-[blue]"
          } font-[main] `}
        >
          {statusText}
        </span>
